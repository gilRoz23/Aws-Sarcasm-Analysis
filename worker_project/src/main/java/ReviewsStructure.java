...
public class ReviewsStructure {
    ...
    private ReviewsStructure() {
    }

    public static ReviewsStructure StringToReviewsStructure(String reviewString) {
        JSONParser parser = new JSONParser();
        ReviewsStructure cur = new ReviewsStructure();
        cur.reviews = new LinkedList<Review>();
        try {
            Object jsonObj = parser.parse(reviewString);
            JSONObject jsonObject = (JSONObject) jsonObj;
            cur.title = (String) jsonObject.get("title");
            Iterator<JSONObject> it = ((JSONArray) jsonObject.get("reviews")).iterator(); // the reason is that hte
                                                                                          // value of
                                                                                          // jsonObject.get(reviews)=
                                                                                          // List(jasonObjects)
            while (it.hasNext()) {
    ...
    public static List<ReviewsStructure> FileToInput(String path) {
        List<ReviewsStructure> ans = new LinkedList<ReviewsStructure>();
