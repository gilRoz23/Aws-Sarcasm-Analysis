...
public class StanfordActivator {
    ...
    public analyzedReview process(Review review) {
        List<String> entities = getEntities(review.getText());
        int sentiment = findSentiment(review.getText());
        return new analyzedReview(review.getLink(), convertSentimentRatingToColor(sentiment), entities, isSarcasm(review.getRating(), sentiment));
    }
